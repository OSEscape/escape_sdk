...
from escape_sdk.types.box import Box, createGrid
from escape_sdk.types.item import Item, ItemIdentifier
from escape_sdk.types.itemcontainer import ItemContainer
from escape_sdk.types.widget import Widget, WidgetFields
from escape_sdk.utilities import timing
...
class Bank(ItemContainer):
    ...
    Example:
        from escape_sdk.interfaces.bank import bank
    ...
    def getOpenTab(self) -> int | None:
        ...
        Example:
            ...
            if tab is not None:
                logger.debug("Current bank tab: %s", tab)
        """
        if not self.isOpen():
            return None
    ...
    def setNotedMode(self, noted: bool) -> bool:
        ...
        logger.debug("Setting noted mode to %s, currently %s", noted, currently_noted)
    ...
    def getItemBox(self, identifier: ItemIdentifier) -> Box | None:
        ...
        try:
            logger.debug("Bank items result: %s", result)
            if result["isHidden"]:
                return None
            ...
            logger.debug("Error getting item area: %s", e)
            return None
    ...
    def checkItemsDeposited(self, start_count) -> bool:
        ...
        logger.debug("start count: %s, current count: %s", start_count, current_count)
