        ...
        *args,
        ...
        if declaring_class:
            ...
            if method_info:
                ...
                logger.warning(
                    "Could not resolve declaring_class for '%s' on type '%s'",
                    method_name,
                    target_class,
                )
        ...
        if "declaring_class" not in operation or operation["declaring_class"] is None:
            logger.error(
                "declaring_class is missing for method '%s' - this will cause JNI bridge to crash. Operation: %s",
                method_name,
                operation,
            )
            raise RuntimeError(
...
class Query:
        ...
        if isinstance(data, dict):
            ...
            if "$enum" in data:
                ...
                if enum_class_name in self.api.enum_classes:
                    ...
                    logger.debug("Unknown enum class: %s", enum_class_name)
                    return data
            else:
    ...
    def _executeQuery(self, operations: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if hasattr(self.api, "executeBatchQuery"):
            ...
            logger.debug("Would execute query with %d operations", len(operations))
    ...
    def loop(self, container_ref: QueryRef, start: int, end: int, item_func: Callable) -> QueryRef:
        ...
        if item_func and self.lambda_compiler:
            try:
                ...
                logger.warning("Failed to compile item function: %s", e)
    ...
    def forEach(
        ...
        if transform_func:
            try:
                            ...
                            if has_chains:
                                ...
                                for expr in expressions:
                                    ...
                                    if methods_chain:
                                        ...
                                        for method in methods_chain:
                                            ...
                                            if method_info:
                                                ...
                                                if method in self.api.api_data["methods"]:
                                                    ...
                                                    logger.warning(
                                                        "Method '%s' not found for type '%s'. Using current_type as declaring_class.",
                                                        method,
                                                        current_type,
                                                    )
                                                    chain_ops.append(
                                        ...
                                        if missing_methods:
                                            logger.error(
                                                "Method(s) not found in RuneLite API: %s. Check spelling and API documentation.",
                                        ...
                                        if method_info:
                                            ...
                                            if is_external:
                                                ...
                                                if (
                                                    ...
                                                    fallback_sig = any_entry[1]
                                                    logger.warning(
                                                        "Calling '%s' on external type '%s'. Last method in chain - JNI will resolve at runtime.",
                                                        method,
                                                        current_type,
                                                    )
                                                    ...
                                                    logger.error(
                                                        "Cannot chain methods on external Java class '%s'. "
                                                        "Method '%s' returns a type not in the scraped RuneLite API. "
                                                        "Chaining is only supported on RuneLite API types. Chain: %s",
                                                        current_type,
                                                        method,
                                                        " -> ".join(methods_chain),
                                                    )
                                                ...
                                                logger.error(
                                                    "Method '%s' not found in RuneLite API",
                                                    method,
                                                )
                ...
                logger.debug("Lambda source parsing failed: %s. Attempting bytecode analysis fallback...", e)
                ...
                try:
                    ...
                    logger.debug("Bytecode analysis found methods: %s", method_calls)
                    ...
                    if method_calls:
                        ...
                        if transform_ops:
                            logger.debug("Successfully built transform_ops from bytecode!")
                        else:
                            logger.debug("Could not resolve signatures for methods: %s", method_calls)
                except Exception as bytecode_err:
                    logger.debug("Bytecode analysis also failed: %s", bytecode_err)
        ...
        if transform_ops:
                ...
                for i, op in enumerate(transform_ops):
                    if isinstance(op, dict) and op.get("type") == "method":
                        if "declaring_class" not in op or op["declaring_class"] is None:
                            logger.error(
                                "Transform operation #%d missing declaring_class for method '%s'. This will cause JNI bridge to crash.",
                                i,
                                op.get("method"),
                            )
                            raise RuntimeError(
                        ...
                        for j, chain_op in enumerate(op):
                            if isinstance(chain_op, dict) and chain_op.get("type") == "method":
                                if (
                                    ...
                                    logger.error(
                                        "Chain operation #%d.%d missing declaring_class for method '%s'",
                                        i,
                                        j,
                                        chain_op.get("method"),
                                    )
            ...
            logger.warning(
                "forEach lambda parsing failed. Unable to extract method calls from lambda. "
                ...
                "Try defining the lambda in a .py file instead."
            )
