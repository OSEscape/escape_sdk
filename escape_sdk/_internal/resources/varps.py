...
def getVarpByName(name: str) -> int | None:
    ...
    """
    if not _varps_data:
        logger.warning("Varps data not loaded")
        return None
    ...
    logger.debug("Varp '%s' not found", name)
    return None
...
def getVarbitByIndex(varbit_id: int) -> int | None:
    ...
    """
    if not _varbits_data:
        logger.warning("Varbits data not loaded")
        return None

    # Get varbit info
    varbit_info = _varbits_data.get(varbit_id)
    if not varbit_info:
        logger.debug("Varbit %d not found", varbit_id)
        return None

    # Get the varp value
    varp_id = varbit_info.get("varp")
    if varp_id is None:
        logger.debug("Varbit %d has no varp mapping", varbit_id)
        return None
...
def getVarbitByName(name: str) -> int | None:
    ...
    logger.debug("Varbit '%s' not found", name)
    return None
