...
class ProtocolEncoder:
    ...
    def encodeRequest(operations: List[Dict]) -> bytes:
        """Encode operations for bridge protocol."""
        processed = []

        for op in operations:
            new_op = {}

            # Operation type (method, static, construct, field_get, etc.)
            if "type" in op:
                new_op["type"] = op["type"]

            # Operation reference ID (for result correlation)
            if "ref" in op:
                new_op["ref"] = op["ref"]
            ...
            # Declaring class for method dispatch
            if "declaring_class" in op:
                new_op["declaring_class"] = op["declaring_class"]
            ...
            # Signature only - bridge parses it (single source of truth)
            if "signature" in op:
                new_op["signature"] = op["signature"]

            # Process arguments with type encoding
            if "args" in op:
                encoded_args = []
                # Parse signature for type hints (local use only, not sent)
                arg_types = []
                if "signature" in op:
                    type_info = ProtocolEncoder.parseSignature(op["signature"])
                    arg_types = type_info["arg_types"]
