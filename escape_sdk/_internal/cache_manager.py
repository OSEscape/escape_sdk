"""
Centralized cache management for Escape SDK.

Uses ~/.cache/escape_sdk/ for all generated files and resources following XDG Base Directory specification.
...
class CacheManager:
    """Manages cache directories for Escape SDK resources and generated files."""
    ...
    def __init__(self, base_path: Path | None = None):
        ...
        Args:
            base_path: Optional custom base path. If None, uses ~/.cache/escape_sdk
        """
        if base_path is None:
            # Use XDG_CACHE_HOME if set, otherwise default to ~/.cache
            xdg_cache = os.getenv("XDG_CACHE_HOME")
            if xdg_cache:
                self.base_path = Path(xdg_cache) / "escape_sdk"
            else:
                self.base_path = Path.home() / ".cache" / "escape_sdk"
        else:
    ...
    def getGeneratedPath(self, filename: str) -> Path:
        ...
        Returns:
            Full path to the file in ~/.cache/escape_sdk/generated/
        """
    ...
    def getObjectsPath(self) -> Path:
        ...
        Returns:
            Path to ~/.cache/escape_sdk/data/objects/
        """
    ...
    def getVarpsPath(self) -> Path:
        ...
        Returns:
            Path to ~/.cache/escape_sdk/data/varps/
        """
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=30) as response:
            ...
            if decompress_gz and url.endswith(".gz"):
                ...
                logger.debug("Downloaded: %s bytes", f"{gz_file.stat().st_size:,}")

                # Decompress to working file
                logger.debug("Decompressing %s...", gz_file.name)
                with gzip.open(gz_file, "rb") as f_in:
                    with open(dest, "wb") as f_out:
                        shutil.copyfileobj(f_in, f_out)

                logger.debug("Decompressed: %s bytes", f"{dest.stat().st_size:,}")

                # Delete the .gz file after decompression
                gz_file.unlink()
                logger.debug("Removed compressed file")
            else:
                # Write directly
                with open(dest, "wb") as f:
                    f.write(data)
                logger.debug("Downloaded: %s bytes", f"{dest.stat().st_size:,}")
...
def _needsUpdate(cache_dir: Path) -> bool:
    ...
    if remote_revision and local_revision and remote_revision != local_revision:
        logger.info("New revision available: %s -> %s", local_revision, remote_revision)
        return True
...
def ensureResourcesLoaded() -> bool:
    ...
    try:
        ...
        if _needsUpdate(cache_dir):
            ...
            for local_name, remote_name in files.items():
                ...
                if not _downloadFile(url, dest, decompress_gz=decompress):
                    logger.error("Failed to download %s", local_name)
                    return False

            logger.info("Resources downloaded successfully")

        # Load resources into modules
        from escape_sdk._internal.resources import objects, varps
        ...
        with open(varps_file) as f:
            ...
            logger.debug("Loaded %d varps", len(varps._varps_data))
        ...
        with open(varbits_file) as f:
            ...
            logger.debug("Loaded %d varbits", len(varps._varbits_data))
        ...
        logger.debug("Loaded objects database")
...
def ensureGeneratedFiles():
    ...
    if not hasGeneratedFiles():
        logger.info("Generated files not found in cache, running updater...")
        try:
            ...
            if not success or not hasGeneratedFiles():
                raise FileNotFoundError(
                    ...
                    "Run 'python -m escape_sdk._internal.updater --force' manually."
                )
        except Exception as e:
            raise FileNotFoundError(
                f"Could not generate required files: {e}\n"
                f"Run 'python -m escape_sdk._internal.updater --force' manually."
            ) from e
