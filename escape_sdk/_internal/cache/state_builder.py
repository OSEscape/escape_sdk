...
import escape_sdk.utilities.timing as timing
from escape_sdk._internal.events.channels import LATEST_STATE_CHANNELS
from escape_sdk._internal.resources import varps as varps_resource
from escape_sdk.globals import getApi
from escape_sdk.types import Item, ItemContainer
...
class StateBuilder:
    ...
    def addEvent(self, channel: str, event: Dict[str, Any]) -> None:
        ...
        if channel in [
            ...
            logger.debug("Processing event on channel %s: %s", channel, event)
    ...
    def initGroundItems(self) -> None:
        ...
        try:
            ...
            logger.error("Rebuild grounditems failed: %s", e)
            return

    def _processCameraChanged(self) -> None:
        """Invalidate tile projection cache when camera changes."""
        from escape_sdk.world.projection import projection
    ...
    def _processWorldViewLoaded(self, event: Dict[str, Any]) -> None:
        ...
        Args:
            event: World view loaded event with keys:
                ...
                - plane: Current plane
        """
        from escape_sdk.world.projection import EntityConfig, projection
